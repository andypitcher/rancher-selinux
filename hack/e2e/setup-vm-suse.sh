...
function enforceSELinux(){
    ...
    sudo zypper install -y container-selinux selinux-policy
    # Install iptables (required for RKE2 nginx ingress on openSUSE).
    sudo zypper install -y iptables
    # Install rancher-selinux policy.
    sudo zypper install -y /tmp/rancher-selinux.rpm
}
